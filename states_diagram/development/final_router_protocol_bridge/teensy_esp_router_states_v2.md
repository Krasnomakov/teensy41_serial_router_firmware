---
diagram: teensy-esp-router-v2
version: 0.3.0
updated: 2025-10-13
author: GLOW Systems Team
description: Router orchestration updated for the 2025 firmware refresh with ESP confirmations and router-level acknowledgements.
---
...
| S_CONF_209_START_CLIMAX_TOP | CONFIRM_START_CLIMAX_TOP | TOP | confirm | Top piece confirms launch sequence |
...
| --- | --- | --- | --- | --- |
| S_IDLE_ROUTER | mic_start_event | ID101 MAKE_STAR | S_REQ_101_MAKE_STAR | MAC selects target arm; router prepends MASTER when forwarding |
...
| S_REQ_104_STAR_ARRIVED | router_ack_to_esp | ID104 STAR_ARRIVED | S_REQ_104_STAR_ARRIVED | Router emits CONFIRM:STAR_ARRIVED{status=received} back to ESP before returning to MAC |
...
| S_REQ_107_CLIMAX_READY | router_ack_to_esp | ID107 CLIMAX_READY | S_REQ_107_CLIMAX_READY | Router replies to ESP with CONFIRM:CLIMAX_READY{status=forwarded} before relaying to MAC |
...
| S_ERR_500_COMM_ERROR | router_ack_to_esp | ID500 COMM_ERROR | S_ERR_500_COMM_ERROR | Router confirms receipt to ESP while escalating to MAC |
...
| ID104 | STAR_ARRIVED | ARM↔ROUTER↔MAC | arm_id,status | Arm reports star arrival; router echoes confirm to ESP before forwarding |
...
| ID107 | CLIMAX_READY | CENTER↔ROUTER↔MAC | device_id,status | Centerpiece signals readiness; router confirms receipt to ESP while relaying |
...
| ID208 | START_CLIMAX_CENTER_CONFIRM | CENTER→ROUTER→MAC | request_id,status | Centerpiece confirms climax sequence |
| ID209 | START_CLIMAX_TOP_CONFIRM | TOP→ROUTER→MAC | request_id,status | Top piece confirms climax launch |
...
| ID500 | COMM_ERROR | ROUTER↔MAC↔ESP | device_id,error_code,error_text | Router reports communication failure and confirms receipt with ESP |
...
- ESP scripted responses now emit automated follow-ups (`STAR_ARRIVED`, `CLIMAX_READY`) that the router forwards upstream while issuing confirmations back to the ESP link.
- Router acknowledgements toward the ESP ensure manual injection tests on a single serial console still reveal round-trip health.
- Frame parsing tolerates incidental whitespace at the end of a manual frame before the `##` terminator, aligning the docs with firmware safeguards.

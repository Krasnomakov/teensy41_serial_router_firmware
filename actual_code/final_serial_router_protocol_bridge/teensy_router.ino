#include "ProtocolParser.h" // Shared parsing logic
#include <Arduino.h>
#include <ctype.h>

// MessageParts and helpers removed - now in ProtocolParser.h
...
static bool TRACE_SERIAL1_RX_BYTES =
    true;                               // Toggle raw ESP->Teensy byte logging
...
// -------------------------------------------------------------------------------------------------
// Pending command tracking (for routing confirmations back to the correct
// device)
// -------------------------------------------------------------------------------------------------
...
static void forwardToEsp(const MessageParts &parts) {
  ...
  // Insert MASTER at the front of the address chain when routing to the ESP
  // device.
...
static void forwardToMac(const MessageParts &parts,
                         const String &fallbackDevice) {
  MessageParts outbound = parts;
...
static void sendAckToEsp(const String &deviceTag, const String &command,
                         const char *status) {
  MessageParts ack;
  ack.addressCount = 1;
...
static void handleMacMessage(const String &raw) {
  ...
  const String targetDevice =
      parts.addressCount > 0 ? parts.addresses[0] : String();
...
static void handleEspMessage(const String &raw) {
  ...
  } else if (commandKey == "CLIMAX_READY" &&
             parts.type.equalsIgnoreCase("REQUEST")) {
...
static void pumpStream(Stream &stream, String &buffer, bool fromMac,
                       bool traceBytes) {
